...
/* variables from init made available to run */

struct frank_init_data
{
  ulong tile_cnt;
...
/* init */
void
frank_init( int * pargc, 
            char *** pargv,
            void * data ) {

  struct frank_init_data * init_data = (struct frank_init_data *) data;
  ...
  init_data->pod_gaddr = fd_env_strip_cmdline_cstr( pargc, pargv, "--pod", NULL, NULL );

  if( FD_UNLIKELY( !init_data->pod_gaddr ) ) FD_LOG_ERR(( "--pod not specified" ));

  /* Load up the configuration for this frank instance */

  FD_LOG_NOTICE(( "using configuration in pod --pod %s at path %s", init_data->pod_gaddr, FD_FRANK_CONFIGURATION_PREFIX ));

  init_data->pod = fd_wksp_pod_attach( init_data->pod_gaddr );
  uchar const * cfg_pod = fd_pod_query_subpod( init_data->pod, FD_FRANK_CONFIGURATION_PREFIX );
  ...
  init_data->tile_cnt = 3UL + verify_cnt;

  if ( init_data->tile_cnt > FD_MAX_TILE_CNT ) {
    FD_LOG_ERR(( "requested %lu tiles while %lu is max", init_data->tile_cnt, FD_MAX_TILE_CNT ));
  }

  /* ensure there are enough tiles */
  if( FD_UNLIKELY( fd_tile_cnt()<init_data->tile_cnt ) ) FD_LOG_ERR(( "at least %lu tiles required for this config - has %lu", init_data->tile_cnt, fd_tile_cnt() ));
  if( FD_UNLIKELY( fd_tile_cnt()>init_data->tile_cnt ) ) FD_LOG_WARNING(( "only %lu tiles required for this config - has %lu", init_data->tile_cnt, fd_tile_cnt() ));

  /* Join all IPC objects needed by main */
  do {
    ulong tile_idx = 0UL;
    printf( "tile_name: %p\n", (void*) init_data->tile_name);
    FD_LOG_NOTICE(( "joining %s.main.cnc", FD_FRANK_CONFIGURATION_PREFIX ));
    init_data->tile_name[ tile_idx ] = "main";
    init_data->tile_cnc [ tile_idx ] = fd_cnc_join( fd_wksp_pod_map( cfg_pod, "main.cnc" ) );
    if( FD_UNLIKELY( !init_data->tile_cnc[ tile_idx ] ) ) FD_LOG_ERR(( "fd_cnc_join failed" ));
    if( FD_UNLIKELY( fd_cnc_app_sz( init_data->tile_cnc[ tile_idx ] )<64UL ) ) FD_LOG_ERR(( "cnc app sz should be at least 64 bytes" ));
    tile_idx++;

    FD_LOG_NOTICE(( "joining %s.pack.cnc", FD_FRANK_CONFIGURATION_PREFIX ));
    init_data->tile_name[ tile_idx ] = "pack";
    init_data->tile_cnc [ tile_idx ] = fd_cnc_join( fd_wksp_pod_map( cfg_pod, "pack.cnc" ) );
    if( FD_UNLIKELY( !init_data->tile_cnc[ tile_idx ] ) ) FD_LOG_ERR(( "fd_cnc_join failed" ));
    if( FD_UNLIKELY( fd_cnc_app_sz( init_data->tile_cnc[ tile_idx ] )<64UL ) ) FD_LOG_ERR(( "cnc app sz should be at least 64 bytes" ));
    tile_idx++;

    FD_LOG_NOTICE(( "joining %s.dedup.cnc", FD_FRANK_CONFIGURATION_PREFIX ));
    init_data->tile_name[ tile_idx ] = "dedup";
    init_data->tile_cnc [ tile_idx ] = fd_cnc_join( fd_wksp_pod_map( cfg_pod, "dedup.cnc" ) );
    if( FD_UNLIKELY( !init_data->tile_cnc[ tile_idx ] ) ) FD_LOG_ERR(( "fd_cnc_join failed" ));
    if( FD_UNLIKELY( fd_cnc_app_sz( init_data->tile_cnc[ tile_idx ] )<64UL ) ) FD_LOG_ERR(( "cnc app sz should be at least 64 bytes" ));
    tile_idx++;
    ...
    for( fd_pod_iter_t iter = fd_pod_iter_init( verify_pods ); !fd_pod_iter_done( iter ); iter = fd_pod_iter_next( iter ) ) {
      ...
      init_data->tile_name[ tile_idx ] = verify_name;
      init_data->tile_cnc [ tile_idx ] = fd_cnc_join( fd_wksp_pod_map( verify_pod, "cnc" ) );
      if( FD_UNLIKELY( !init_data->tile_cnc[tile_idx] ) ) FD_LOG_ERR(( "fd_cnc_join failed" ));
      if( FD_UNLIKELY( fd_cnc_app_sz( init_data->tile_cnc[ tile_idx ] )<64UL ) ) FD_LOG_ERR(( "cnc app sz should be at least 64 bytes" ));
      tile_idx++;
    }
...
main( int argc, 
  ...
  FD_LOG_NOTICE(( "entering run" ));

  /* allocate space for data that must be passed from init to run */
  struct frank_init_data * init_data = fd_alloca( alignof( struct frank_init_data ), sizeof( struct frank_init_data ) );
  init_data->tile_name = fd_alloca( alignof( char* ), sizeof( char* ) * FD_TILE_MAX );
  init_data->tile_cnc = fd_alloca( alignof( fd_cnc_t * ), sizeof(fd_cnc_t * ) * FD_TILE_MAX );

  /* boot firedancer and execute frank_init in an unrestricted fashion */
  fd_boot_secure( &argc, &argv, frank_init, (void *) init_data );

  /* Boot all the tiles that main controls */

  for( ulong tile_idx=1UL; tile_idx<init_data->tile_cnt; tile_idx++ ) {
    FD_LOG_NOTICE(( "booting tile %s", init_data->tile_name[ tile_idx ] ));
    ...
    char * task_argv[3];
    task_argv[0] = (char *)init_data->tile_name[ tile_idx ];
    task_argv[1] = (char *)init_data->pod_gaddr;
    ...
    if( FD_UNLIKELY( fd_cnc_wait( init_data->tile_cnc[ tile_idx ], FD_CNC_SIGNAL_BOOT, (long)5e9, NULL )!=FD_CNC_SIGNAL_RUN ) )
  ...
  fd_cnc_t * cnc = FD_VOLATILE( fd_frank_main_cnc ) = init_data->tile_cnc[ 0 ];
  if( FD_UNLIKELY( fd_cnc_signal_query( cnc )!=FD_CNC_SIGNAL_BOOT ) ) FD_LOG_ERR(( "cnc not in boot state" ));
  ulong * cnc_diag = (ulong *)fd_cnc_app_laddr( init_data->tile_cnc[ 0 ] );
  ...
  for( ulong tile_idx=init_data->tile_cnt; tile_idx>1UL; tile_idx-- ) {
    FD_LOG_NOTICE(( "halting tile %s", init_data->tile_name[ tile_idx-1UL ] ));

    /* Note: could do this in parallel too but doing reverse
       one-at-a-time for symmetry with boot */

    if( FD_UNLIKELY( fd_cnc_open( init_data->tile_cnc[ tile_idx-1UL ] ) ) ) FD_LOG_ERR(( "fd_cnc_open failed for tile %lu", tile_idx-1UL ));
    fd_cnc_signal( init_data->tile_cnc[ tile_idx-1UL ], FD_CNC_SIGNAL_HALT );
    fd_cnc_close ( init_data->tile_cnc[ tile_idx-1UL ] );
  ...
  for( ulong tile_idx=init_data->tile_cnt; tile_idx; tile_idx-- ) fd_wksp_pod_unmap( fd_cnc_leave( init_data->tile_cnc[ tile_idx-1UL ] ) );
  fd_wksp_pod_detach( init_data->pod );
  fd_halt();
  return 0;
}
