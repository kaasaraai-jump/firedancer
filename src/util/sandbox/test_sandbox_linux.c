...
test_should_close_open_fds( void ) {
  ...
  fd_sandbox_close_fds_beyond( (uint) fd - 1 );
...
int
test_should_change_userns( void ) {
  // pid_t pid, w;
  ...
  // } else { // child
  //     struct passwd * uinfo = getpwnam("nobody");
  ...
  //     if ( FD_UNLIKELY ( strcmp( "nobody", current_user->pw_name ) ) ) 
  //       FD_LOG_ERR(( "expected current user to be 'nobody'" ));
...
/* test_should_set_resource_limits ensures that, after calling `fd_sandbox_set_resource_limits`,
...
test_should_set_resource_limits( void ) {
  ...
  if (( pid = fork() )) { // parent
    ...
    if ( FD_UNLIKELY ( WIFEXITED( wstatus ) && WEXITSTATUS( wstatus ) ) ) {
      FD_LOG_WARNING(( "expected fd_sandbox_set_resource_limits to exit gracefully but " "exited with non-zero status, status=%d", WEXITSTATUS(wstatus) ));
      return -1;
    } else if ( FD_UNLIKELY ( WIFSIGNALED( wstatus ) ) ) {
      FD_LOG_WARNING(( "expected fd_sandbox_set_resource_limits to exit gracefully but " "killed by signal %s", strsignal(WTERMSIG(wstatus)) ));
      return -1;
    } else if ( FD_UNLIKELY ( WIFSTOPPED( wstatus ) ) ) {
      FD_LOG_WARNING(( "expected fd_sandbox_set_resource_limits to exit gracefully but " "stopped by signal %s", strsignal(WSTOPSIG(wstatus)) ));
      return -1;
    }
    return 0;
    ...
    fd_sandbox_set_resource_limits(3U);
...
/* test_setup_netns ensures that, after calling `fd_sandbox_setup_netns`,
...
test_setup_netns( void ) {
  ...
  if (( pid = fork() )) { // parent
    ...
    if ( FD_UNLIKELY ( WIFEXITED( wstatus ) && WEXITSTATUS( wstatus ) ) ) {
      FD_LOG_WARNING(("expected fd_sandbox_setup_netns to exit gracefully but " "exited with non-zero status, status=%d", WEXITSTATUS(wstatus) ));
      return -1;
    } else if ( FD_UNLIKELY ( WIFSIGNALED( wstatus ) ) ) {
        FD_LOG_WARNING(( "expected fd_sandbox_setup_netns to exit gracefully but " "killed by signal %s", strsignal(WTERMSIG(wstatus)) ));
        return -1;
    } else if ( FD_UNLIKELY ( WIFSTOPPED( wstatus ) ) ) {
        FD_LOG_WARNING(( "expected fd_sandbox_setup_netns to exit gracefully but " "stopped by signal %s", strsignal(WSTOPSIG(wstatus)) ));
        return -1;
    }
    return 0;

  } else { // child
    fd_sandbox_setup_netns( nsname );
...
/* test_seccomp_default_filter ensures that, after calling `fd_sandbox_seccomp`,
...
/* test_setup_netns_null ensures that, after calling `fd_sandbox_setup_mountns`,
...
main( int     argc,
  ...
  if( FD_UNLIKELY ( test_should_change_userns() ) ) {
    failed_cnt++;
